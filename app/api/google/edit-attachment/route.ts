...
// Converts an existing job attachment into an editable Google Doc inside the
// job's Drive folder, so it can be opened and edited with full Google Docs
// design tools. Returns the Google Docs link + file id (the file id can later
// be passed to /api/google/save-doc-to-job to capture edits back into the job).
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const doc = await uploadAsGoogleDoc(accessToken, fileName, folderId ?? undefined, bytes, sourceMimeForFile(fileName));

    return NextResponse.json({ url: doc.webViewLink, docId: doc.id, docName: fileName });
  } catch (e) {
