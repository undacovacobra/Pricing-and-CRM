...
export function TemplateDocumentEditor({ jobId, title, template, googleReady }: Props) {
  ...
  async function recordToJob(storagePath: string, fileName: string) {
    ...
    // Best-effort mirror into the job's Google Drive folder.
    if (googleReady) {
      try {
        const res = await fetch("/api/google/save-to-drive", {
          method:  "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ jobId, storagePath, fileName }),
        });
        if (res.ok) setDriveSynced(true);
      } catch {
        // Non-fatal: the file is already saved to the job either way.
      }
    }
  }
    ...
    return (
      <div className="border rounded-lg p-4 bg-green-50 border-green-200 space-y-3">
        <p className="text-sm text-green-800">
          Saved to this job&apos;s attachments{driveSynced ? " and its Google Drive folder" : ""}, and logged in notes.
        </p>
    ...
    <div className="space-y-4">
      {!googleReady && (
        <div className="text-xs text-muted-foreground bg-slate-50 border rounded-md px-3 py-2">
          Tip: <Link href="/settings" className="text-blue-600 underline">connect Google Drive in Settings</Link> so
          saved documents are also mirrored into this job&apos;s Drive folder.
        </div>
      )}
        ...
        {editable ? (
          <>
            <p className="text-xs text-muted-foreground">
              Editing a copy of <span className="font-medium">{template.file_name}</span>. Make your changes below,
              then save — it will be stored in this job&apos;s attachments{googleReady ? " and Drive folder" : ""} and
              logged in notes.
            </p>
            {loading ? (
